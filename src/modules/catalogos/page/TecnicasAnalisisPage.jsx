...
import {
  ...
  getTecnicasAnalisis,
  toggleTecnicaAnalisisStatus,
  updateTecnicaAnalisis,
} from "../service/tecnicasAnalisisService.js";

const initialForm = { nombreTecnica: "", descripcionTecnica: "", idLaboratorio: "" };

function validateField(name, value) {
  if (name === "descripcionTecnica") return "";
  if (!String(value ?? "").trim()) return "Este campo es requerido";
  return "";
}

export default function TecnicasAnalisisPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [tecnicasData, labsData] = await Promise.all([getTecnicasAnalisis(), getLaboratorios()]);
      setItems(tecnicasData);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) =>
      i.nombreTecnica?.toLowerCase().includes(q) ||
      i.descripcionTecnica?.toLowerCase().includes(q) ||
  ...
  function isFormValid() {
    return !validateField("nombreTecnica", form.nombreTecnica) &&
      !validateField("idLaboratorio", form.idLaboratorio);
  }
  ...
  function toPayload(activo = true) {
    return {
      nombreTecnica: form.nombreTecnica,
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        await updateTecnicaAnalisis(editing.idTecnicaAnalisis, toPayload(editing.activo));
        addToast("Técnica de análisis actualizada exitosamente", "success");
      } else {
        await createTecnicaAnalisis(toPayload(true));
        addToast("Técnica de análisis creada exitosamente", "success");
      }
      closeModal();
      await loadData();
    } catch (err) {
  ...
  async function handleToggleStatus(item) {
    setTogglingId(item.idTecnicaAnalisis);
    try {
      await toggleTecnicaAnalisisStatus(item);
      addToast(item.activo ? "Técnica desactivada exitosamente" : "Técnica activada exitosamente", "success");
      setItems((p) => p.map((i) => i.idTecnicaAnalisis === item.idTecnicaAnalisis ? { ...i, activo: !i.activo } : i));
    } catch (err) {
      addToast(err.message, "error");
    } finally {
      setTogglingId(null);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        <button type="button" onClick={openCreateModal}
          ...
          <FaPlus className="h-4 w-4" /> Nueva Técnica de Análisis
        </button>
      </div>

      <div className="relative">
        <FaSearch className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-blue-900" />
        <input type="text" placeholder="Buscar por nombre, descripción o laboratorio..." value={search}
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                ...
                <span className="mt-2 block">Cargando técnicas de análisis...</span>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                {search ? "No se encontraron técnicas" : "No hay técnicas de análisis registradas"}
              </td></tr>
            ) : filtered.map((item) => (
              <tr key={item.idTecnicaAnalisis} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900 sm:px-6">{item.nombreTecnica}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="relative inline-flex cursor-pointer items-center">
                      <input type="checkbox" className="peer sr-only" checked={item.activo}
                        disabled={togglingId === item.idTecnicaAnalisis}
                    ...
                    {togglingId === item.idTecnicaAnalisis && <FaSpinner className="h-4 w-4 animate-spin text-gray-400" />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">{editing ? "Editar Técnica de Análisis" : "Nueva Técnica de Análisis"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
              <InputField label="Nombre de la Técnica" name="nombreTecnica" value={form.nombreTecnica}
                error={formErrors.nombreTecnica} onChange={handleFormChange} required />
              <TextAreaField label="Descripción" name="descripcionTecnica" value={form.descripcionTecnica}
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">Detalle de la técnica de análisis</h2>
            ...
            <div className="space-y-4 p-6">
              <dl className="space-y-3">
                <DetailRow label="Nombre" value={detailItem.nombreTecnica} />
                <DetailRow label="Descripción" value={detailItem.descripcionTecnica} />
                ...
                <DetailRow label="ID Técnica" value={detailItem.idTecnicaAnalisis} />
              </dl>
      ...
      <ConfirmDialog open={!!confirmToggle} title="Cambiar Estado"
        message={`¿Está seguro de que desea ${confirmToggle?.activo ? "desactivar" : "activar"} la técnica "${confirmToggle?.nombreTecnica || ""}"?`}
...
function TextAreaField({ label, name, value, onChange }) {
  ...
  return (
    <div>
      ...
      <textarea id={id} name={name} value={value} onChange={onChange} rows={3} className="input" />
    </div>
  );
}

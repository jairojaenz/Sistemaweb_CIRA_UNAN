/**
 * Catálogo visual de Análisis (CRUD + activar/desactivar).
 * Mismo patrón que GruposAnalisisPage / TecnicasAnalisisPage.
 */
...
import {
  createAnalisis,
  getAnalisis,
  toggleAnalisisStatus,
  updateAnalisis,
} from "../service/analisisService.js";
import { getGruposAnalisis } from "../service/gruposAnalisisService.js";
...
function validateField(name, value) {
  // Descripción y grupo son opcionales; el resto no puede ir vacío.
  if (name === "descripcionAnalisis" || name === "idGrupoAnalisis") return "";
  if (name === "precioAnalisis") {
...
export default function AnalisisPage() {
  ...
  const grupoMap = useMemo(() => {
    const map = {};
    grupos.forEach((g) => { map[g.idGrupoAnalisis] = g.nombreGrupo; });
    return map;
  }, [grupos]);
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [analisisData, labsData, gruposData] = await Promise.all([
        getAnalisis(),
        getLaboratorios(),
        getGruposAnalisis(),
      ]);
      setItems(analisisData);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) =>
      i.nombreAnalisis?.toLowerCase().includes(q) ||
      i.abreviacionAnalisis?.toLowerCase().includes(q) ||
      i.nombreLaboratorio?.toLowerCase().includes(q) ||
      labMap[i.idLaboratorio]?.toLowerCase().includes(q) ||
      grupoMap[i.idGrupoAnalisis]?.toLowerCase().includes(q)
    );
  }, [items, search, labMap, grupoMap]);
  ...
  const grupoOptions = useMemo(
    () => grupos.map((g) => ({ value: String(g.idGrupoAnalisis), label: g.nombreGrupo })),
    [grupos]
  );
  ...
  function isFormValid() {
    return ["nombreAnalisis", "abreviacionAnalisis", "precioAnalisis", "idLaboratorio"].every(
  ...
  function openEditModal(item) {
    setEditing(item);
    setForm({
      nombreAnalisis: item.nombreAnalisis || "",
      abreviacionAnalisis: item.abreviacionAnalisis || "",
      descripcionAnalisis: item.descripcionAnalisis || "",
      precioAnalisis: item.precioAnalisis ?? "",
      idLaboratorio: String(item.idLaboratorio || ""),
      idGrupoAnalisis: item.idGrupoAnalisis ? String(item.idGrupoAnalisis) : "",
    });
    setFormErrors({});
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        await updateAnalisis(editing.idAnalisis, { ...form, activo: editing.activo });
        addToast("Análisis actualizado exitosamente", "success");
      } else {
        await createAnalisis(form);
        addToast("Análisis creado exitosamente", "success");
      }
      closeModal();
      await loadData();
    } catch (err) {
  ...
  async function handleToggleStatus(item) {
    setTogglingId(item.idAnalisis);
    try {
      await toggleAnalisisStatus(item);
      addToast(item.activo ? "Análisis desactivado exitosamente" : "Análisis activado exitosamente", "success");
      setItems((p) => p.map((i) => i.idAnalisis === item.idAnalisis ? { ...i, activo: !i.activo } : i));
    } catch (err) {
      addToast(err.message, "error");
    } finally {
      setTogglingId(null);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        <button type="button" onClick={openCreateModal}
          ...
          <FaPlus className="h-4 w-4" /> Nuevo Análisis
        </button>
      </div>

      <div className="relative">
        <FaSearch className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-blue-900" />
        <input type="text" placeholder="Buscar por nombre, abreviatura o laboratorio..." value={search}
        ...
        <table className="w-full min-w-[800px] text-left text-sm">
          <thead className="bg-gray-50 text-xs uppercase text-gray-600">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold sm:px-6">Abreviatura</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                <FaSpinner className="mx-auto h-6 w-6 animate-spin" />
                <span className="mt-2 block">Cargando análisis...</span>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                {search ? "No se encontraron análisis" : "No hay análisis registrados"}
              </td></tr>
            ) : filtered.map((item) => (
              <tr key={item.idAnalisis} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900 sm:px-6">{item.nombreAnalisis}</td>
                <td className="px-4 py-3 sm:px-6">{item.abreviacionAnalisis}</td>
                <td className="px-4 py-3 sm:px-6">{item.nombreLaboratorio || labMap[item.idLaboratorio] || "—"}</td>
                <td className="px-4 py-3 sm:px-6">C$ {Number(item.precioAnalisis || 0).toFixed(2)}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="relative inline-flex cursor-pointer items-center">
                      <input type="checkbox" className="peer sr-only" checked={item.activo}
                        disabled={togglingId === item.idAnalisis}
                    ...
                    {togglingId === item.idAnalisis && <FaSpinner className="h-4 w-4 animate-spin text-gray-400" />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">{editing ? "Editar Análisis" : "Nuevo Análisis"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
              <InputField label="Nombre" name="nombreAnalisis" value={form.nombreAnalisis}
                error={formErrors.nombreAnalisis} onChange={handleFormChange} required />
              <InputField label="Abreviatura" name="abreviacionAnalisis" value={form.abreviacionAnalisis}
                error={formErrors.abreviacionAnalisis} onChange={handleFormChange} required />
              <InputField label="Descripción" name="descripcionAnalisis" value={form.descripcionAnalisis}
                error={formErrors.descripcionAnalisis} onChange={handleFormChange} />
              <InputField label="Precio" name="precioAnalisis" type="number" step="0.01" min="0"
                value={form.precioAnalisis} error={formErrors.precioAnalisis} onChange={handleFormChange} required />
              ...
              <SelectField label="Grupo de análisis" name="idGrupoAnalisis" value={form.idGrupoAnalisis}
                error={formErrors.idGrupoAnalisis} onChange={handleFormChange} options={grupoOptions} />
              <div className="flex justify-end gap-3 border-t pt-4">
                ...
                <button type="submit" disabled={!isFormValid() || saving}
                  ...
                  {editing ? "Guardar Cambios" : "Crear Análisis"}
                </button>
              </div>
            </form>
          </div>
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">Detalle del análisis</h2>
            ...
            <div className="space-y-4 p-6">
              <dl className="space-y-3">
                <DetailRow label="Nombre" value={detailItem.nombreAnalisis} />
                <DetailRow label="Abreviatura" value={detailItem.abreviacionAnalisis} />
                <DetailRow label="Descripción" value={detailItem.descripcionAnalisis} />
                <DetailRow label="Precio" value={`C$ ${Number(detailItem.precioAnalisis || 0).toFixed(2)}`} />
                <DetailRow label="Laboratorio" value={detailItem.nombreLaboratorio || labMap[detailItem.idLaboratorio]} />
                <DetailRow label="Grupo" value={detailItem.nombreGrupo || grupoMap[detailItem.idGrupoAnalisis]} />
      ...
      <ConfirmDialog open={!!confirmToggle} title="Cambiar Estado"
        message={`¿Está seguro de que desea ${confirmToggle?.activo ? "desactivar" : "activar"} el análisis "${confirmToggle?.nombreAnalisis || ""}"?`}

...
import {
  createTipoMuestreo,
  getTiposMuestreo,
  toggleTipoMuestreoStatus,
  updateTipoMuestreo,
} from "../service/tiposMuestreoService.js";

const initialForm = { nombreTipoMuestreo: "" };
...
export default function TiposMuestreoPage() {
  ...
  const loadItems = useCallback(async () => {
    try {
      setLoading(true);
      setItems(await getTiposMuestreo());
    } catch (err) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => i.nombreTipoMuestreo?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  function isFormValid() {
    return !validateField("nombreTipoMuestreo", form.nombreTipoMuestreo);
  }
  ...
  function openEditModal(item) {
    setEditing(item);
    setForm({ nombreTipoMuestreo: item.nombreTipoMuestreo || "" });
    setFormErrors({});
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        await updateTipoMuestreo(editing.idTipoMuestreo, { ...form, activo: editing.activo });
        addToast("Tipo de muestreo actualizado exitosamente", "success");
      } else {
        await createTipoMuestreo({ ...form, activo: true });
        addToast("Tipo de muestreo creado exitosamente", "success");
      }
      closeModal();
      await loadItems();
    } catch (err) {
  ...
  async function handleToggleStatus(item) {
    setTogglingId(item.idTipoMuestreo);
    try {
      await toggleTipoMuestreoStatus(item.idTipoMuestreo);
      addToast(item.activo ? "Tipo de muestreo desactivado exitosamente" : "Tipo de muestreo activado exitosamente", "success");
      setItems((p) => p.map((i) => i.idTipoMuestreo === item.idTipoMuestreo ? { ...i, activo: !i.activo } : i));
    } catch (err) {
      addToast(err.message, "error");
    } finally {
      setTogglingId(null);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        <button type="button" onClick={openCreateModal}
          ...
          <FaPlus className="h-4 w-4" /> Nuevo Tipo de Muestreo
        </button>
      </div>
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                ...
                <span className="mt-2 block">Cargando tipos de muestreo...</span>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                {search ? "No se encontraron tipos de muestreo" : "No hay tipos de muestreo registrados"}
              </td></tr>
            ) : filtered.map((item) => (
              <tr key={item.idTipoMuestreo} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900 sm:px-6">{item.nombreTipoMuestreo}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="relative inline-flex cursor-pointer items-center">
                      <input type="checkbox" className="peer sr-only" checked={item.activo}
                        disabled={togglingId === item.idTipoMuestreo}
                    ...
                    {togglingId === item.idTipoMuestreo && <FaSpinner className="h-4 w-4 animate-spin text-gray-400" />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">{editing ? "Editar Tipo de Muestreo" : "Nuevo Tipo de Muestreo"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
              <InputField label="Nombre del Tipo de Muestreo" name="nombreTipoMuestreo" value={form.nombreTipoMuestreo}
                error={formErrors.nombreTipoMuestreo} onChange={handleFormChange} required />
              <div className="flex justify-end gap-3 border-t pt-4">
                ...
                <button type="submit" disabled={!isFormValid() || saving}
                  ...
                  {editing ? "Guardar Cambios" : "Crear Tipo de Muestreo"}
                </button>
              </div>
            </form>
          </div>
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">Detalle del tipo de muestreo</h2>
            ...
            <div className="space-y-4 p-6">
              <dl className="space-y-3">
                <DetailRow label="Nombre" value={detailItem.nombreTipoMuestreo} />
                <DetailRow label="Estado" value={detailItem.activo ? "Activo" : "Inactivo"} />
                <DetailRow label="ID Tipo Muestreo" value={detailItem.idTipoMuestreo} />
              </dl>
      ...
      <ConfirmDialog open={!!confirmToggle} title="Cambiar Estado"
        message={`¿Está seguro de que desea ${confirmToggle?.activo ? "desactivar" : "activar"} el tipo de muestreo "${confirmToggle?.nombreTipoMuestreo || ""}"?`}

...
import {
  ...
  toggleCargoStatus,
  updateCargo,
} from "../service/cargosService.js";// Estado inicial del formulario de cargo
...
export default function GestionCargosPage() {
  ...
  }, [addToast]);// Cargar cargos al montar el componente

  useEffect(() => {
    loadCargos();
  }, [loadCargos]);// Filtrar cargos según el término de búsqueda

  const filteredCargos = useMemo(() => {
    const query = search.toLowerCase().trim();
    if (!query) return cargos;
  ...
  }, [cargos, search]); // Manejar cambios en los campos del formulario y validación en tiempo real
  ...
  function isFormValid() {
    ...
    return true;
  }// Abrir modal de creación de cargo
  ...
  function openCreateModal() {
    ...
    setFormErrors({});
    setModalOpen(true);
  }// Editar cargo existente y abrir modal con datos precargados

  function openDetailModal(cargo) {
    setDetailCargo(cargo);
}
...
function closeDetailModal() {
  ...
  async function handleToggleStatus(cargo) {
    setTogglingId(cargo.idCargo);
    try {
      await toggleCargoStatus(
        cargo.idCargo
      );
      addToast(
        cargo.activo
          ? "Cargo desactivado exitosamente"
          : "Cargo activado exitosamente",
        "success"
      );
      setCargos((prev) =>
        prev.map((item) =>

...
const initialForm = { nombreDepartamento: "" };
...
export default function DepartamentosPage() {
  ...
  const loadItems = useCallback(async () => {
    try {
      setLoading(true);
      setItems(await getDepartamentos());
    } catch (err) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => i.nombreDepartamento?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  function isFormValid() {
    return !validateField("nombreDepartamento", form.nombreDepartamento);
  }
  ...
  function openEditModal(item) {
    setEditing(item);
    setForm({ nombreDepartamento: item.nombreDepartamento || "" });
    setFormErrors({});
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        await updateDepartamento(editing.idDepartamento, { ...form, activo: editing.activo });
        addToast("Departamento actualizado exitosamente", "success");
      } else {
        await createDepartamento(form);
        addToast("Departamento creado exitosamente", "success");
      }
      closeModal();
      await loadItems();
    } catch (err) {
  ...
  async function handleToggleStatus(item) {
    setTogglingId(item.idDepartamento);
    try {
      await toggleDepartamentoStatus(item.idDepartamento);
      addToast(item.activo ? "Departamento desactivado exitosamente" : "Departamento activado exitosamente", "success");
      setItems((p) => p.map((i) => i.idDepartamento === item.idDepartamento ? { ...i, activo: !i.activo } : i));
    } catch (err) {
      addToast(err.message, "error");
    } finally {
      setTogglingId(null);
  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6">
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                ...
                <span className="mt-2 block">Cargando departamentos...</span>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                {search ? "No se encontraron departamentos" : "No hay departamentos registrados"}
              </td></tr>
            ) : filtered.map((item) => (
              <tr key={item.idDepartamento} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900 sm:px-6">{item.nombreDepartamento}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="relative inline-flex cursor-pointer items-center">
                      <input type="checkbox" className="peer sr-only" checked={item.activo}
                        disabled={togglingId === item.idDepartamento}
                    ...
                    {togglingId === item.idDepartamento && <FaSpinner className="h-4 w-4 animate-spin text-gray-400" />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">{editing ? "Editar Departamento" : "Nuevo Departamento"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
              <InputField label="Nombre del Departamento" name="nombreDepartamento" value={form.nombreDepartamento}
                error={formErrors.nombreDepartamento} onChange={handleFormChange} required />
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">Detalle del departamento</h2>
            ...
            <div className="space-y-4 p-6">
              <dl className="space-y-3">
                <DetailRow label="Nombre" value={detailItem.nombreDepartamento} />
                <DetailRow label="Estado" value={detailItem.activo ? "Activo" : "Inactivo"} />
                <DetailRow label="ID Departamento" value={detailItem.idDepartamento} />
              </dl>
      ...
      <ConfirmDialog open={!!confirmToggle} title="Cambiar Estado"
        message={`¿Está seguro de que desea ${confirmToggle?.activo ? "desactivar" : "activar"} el departamento "${confirmToggle?.nombreDepartamento || ""}"?`}

...
import {
  createEquipoMuestreo,
  getEquiposMuestreo,
  toggleEquipoMuestreoStatus,
  updateEquipoMuestreo,
} from "../service/equiposMuestreoService.js";

const initialForm = { nombreEquipo: "" };
...
export default function EquiposMuestreoPage() {
  ...
  const loadItems = useCallback(async () => {
    try {
      setLoading(true);
      setItems(await getEquiposMuestreo());
    } catch (err) {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return items;
    return items.filter((i) => i.nombreEquipo?.toLowerCase().includes(q));
  }, [items, search]);
  ...
  function isFormValid() {
    return !validateField("nombreEquipo", form.nombreEquipo);
  }
  ...
  function openEditModal(item) {
    setEditing(item);
    setForm({ nombreEquipo: item.nombreEquipo || "" });
    setFormErrors({});
    setModalOpen(true);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editing) {
        await updateEquipoMuestreo(editing.idEquipo, { ...form, activo: editing.activo });
        addToast("Equipo de muestreo actualizado exitosamente", "success");
      } else {
        await createEquipoMuestreo({ ...form, activo: true });
        addToast("Equipo de muestreo creado exitosamente", "success");
      }
      closeModal();
      await loadItems();
    } catch (err) {
  ...
  async function handleToggleStatus(item) {
    setTogglingId(item.idEquipo);
    try {
      await toggleEquipoMuestreoStatus(item.idEquipo);
      addToast(item.activo ? "Equipo de muestreo desactivado exitosamente" : "Equipo de muestreo activado exitosamente", "success");
      setItems((p) => p.map((i) => i.idEquipo === item.idEquipo ? { ...i, activo: !i.activo } : i));
    } catch (err) {
      addToast(err.message, "error");
    } finally {
      setTogglingId(null);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        <button type="button" onClick={openCreateModal}
          ...
          <FaPlus className="h-4 w-4" /> Nuevo Equipo de Muestreo
        </button>
      </div>
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                ...
                <span className="mt-2 block">Cargando equipos de muestreo...</span>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={3} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                {search ? "No se encontraron equipos de muestreo" : "No hay equipos de muestreo registrados"}
              </td></tr>
            ) : filtered.map((item) => (
              <tr key={item.idEquipo} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900 sm:px-6">{item.nombreEquipo}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="relative inline-flex cursor-pointer items-center">
                      <input type="checkbox" className="peer sr-only" checked={item.activo}
                        disabled={togglingId === item.idEquipo}
                    ...
                    {togglingId === item.idEquipo && <FaSpinner className="h-4 w-4 animate-spin text-gray-400" />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">{editing ? "Editar Equipo de Muestreo" : "Nuevo Equipo de Muestreo"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
              <InputField label="Nombre del Equipo" name="nombreEquipo" value={form.nombreEquipo}
                error={formErrors.nombreEquipo} onChange={handleFormChange} required />
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">Detalle del equipo de muestreo</h2>
            ...
            <div className="space-y-4 p-6">
              <dl className="space-y-3">
                <DetailRow label="Nombre" value={detailItem.nombreEquipo} />
                <DetailRow label="Estado" value={detailItem.activo ? "Activo" : "Inactivo"} />
                <DetailRow label="ID Equipo" value={detailItem.idEquipo} />
              </dl>
      ...
      <ConfirmDialog open={!!confirmToggle} title="Cambiar Estado"
        message={`¿Está seguro de que desea ${confirmToggle?.activo ? "desactivar" : "activar"} el equipo "${confirmToggle?.nombreEquipo || ""}"?`}

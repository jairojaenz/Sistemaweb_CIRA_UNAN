...
import {
  createMunicipio,
  getMunicipios,
  toggleMunicipioStatus,
  updateMunicipio,
} from "../service/municipiosService.js";

const initialForm = { nombreMunicipio: "", idDepartamento: "" };
...
export default function MunicipiosPage() {
  ...
  const deptMap = useMemo(() => {
    const map = {};
    departamentos.forEach((d) => { map[d.idDepartamento] = d.nombreDepartamento; });
    return map;
  }, [departamentos]);
  ...
  const loadData = useCallback(async () => {
    try {
      setLoading(true);
      const [munData, deptData] = await Promise.all([getMunicipios(), getDepartamentos()]);
      setItems(munData);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((i) =>
      i.nombreMunicipio?.toLowerCase().includes(q) ||
      deptMap[i.idDepartamento]?.toLowerCase().includes(q)
    );
  }, [items, search, deptMap]);

  const deptOptions = useMemo(
    () => departamentos.map((d) => ({ value: String(d.idDepartamento), label: d.nombreDepartamento })),
    [departamentos]
  );
  ...
  function isFormValid() {
    return !validateField("nombreMunicipio", form.nombreMunicipio) &&
      !validateField("idDepartamento", form.idDepartamento);
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editing) {
        await updateMunicipio(editing.idMunicipio, { ...payload, activo: editing.activo });
        addToast("Municipio actualizado exitosamente", "success");
      } else {
        await createMunicipio(payload);
        addToast("Municipio creado exitosamente", "success");
      }
      closeModal();
      await loadData();
    } catch (err) {
  ...
  async function handleToggleStatus(item) {
    setTogglingId(item.idMunicipio);
    try {
      await toggleMunicipioStatus(item.idMunicipio);
      addToast(item.activo ? "Municipio desactivado exitosamente" : "Municipio activado exitosamente", "success");
      setItems((p) => p.map((i) => i.idMunicipio === item.idMunicipio ? { ...i, activo: !i.activo } : i));
    } catch (err) {
      addToast(err.message, "error");
    } finally {
      setTogglingId(null);
    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-4 p-4 sm:p-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
        <button type="button" onClick={openCreateModal}
          ...
          <FaPlus className="h-4 w-4" /> Nuevo Municipio
        </button>
      </div>

      <div className="relative">
        <FaSearch className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-blue-900" />
        <input type="text" placeholder="Buscar por nombre o departamento..." value={search}
        ...
        <table className="w-full min-w-[700px] text-left text-sm">
          <thead className="bg-gray-50 text-xs uppercase text-gray-600">
            <tr>
              ...
              <th className="px-4 py-3 font-semibold sm:px-6">Departamento</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {loading ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                ...
                <span className="mt-2 block">Cargando municipios...</span>
              </td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={4} className="px-4 py-10 text-center text-gray-500 sm:px-6">
                {search ? "No se encontraron municipios" : "No hay municipios registrados"}
              </td></tr>
            ) : filtered.map((item) => (
              <tr key={item.idMunicipio} className="hover:bg-gray-50">
                <td className="px-4 py-3 font-medium text-gray-900 sm:px-6">{item.nombreMunicipio}</td>
                <td className="px-4 py-3 text-gray-700 sm:px-6">{deptMap[item.idDepartamento] || "—"}</td>
                  ...
                  <div className="flex items-center gap-2">
                    <label className="relative inline-flex cursor-pointer items-center">
                      <input type="checkbox" className="peer sr-only" checked={item.activo}
                        disabled={togglingId === item.idMunicipio}
                    ...
                    {togglingId === item.idMunicipio && <FaSpinner className="h-4 w-4 animate-spin text-gray-400" />}
                  </div>
                </td>
              </tr>
            ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">{editing ? "Editar Municipio" : "Nuevo Municipio"}</h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 p-6" noValidate>
              <InputField label="Nombre del Municipio" name="nombreMunicipio" value={form.nombreMunicipio}
                error={formErrors.nombreMunicipio} onChange={handleFormChange} required />
              <SelectField label="Departamento" name="idDepartamento" value={form.idDepartamento}
                error={formErrors.idDepartamento} onChange={handleFormChange} required options={deptOptions} />
      ...
      {detailItem && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-6 py-4">
              <h2 className="text-lg font-semibold text-gray-800">Detalle del municipio</h2>
            ...
            <div className="space-y-4 p-6">
              <dl className="space-y-3">
                <DetailRow label="Nombre" value={detailItem.nombreMunicipio} />
                <DetailRow label="Departamento" value={deptMap[detailItem.idDepartamento]} />
                <DetailRow label="Estado" value={detailItem.activo ? "Activo" : "Inactivo"} />
                <DetailRow label="ID Municipio" value={detailItem.idMunicipio} />
              </dl>
      ...
      <ConfirmDialog open={!!confirmToggle} title="Cambiar Estado"
        message={`¿Está seguro de que desea ${confirmToggle?.activo ? "desactivar" : "activar"} el municipio "${confirmToggle?.nombreMunicipio || ""}"?`}
